...
- Expose the extension domain as `cli/lightclient`; keep proof generation as
...
- Remove `cli/app`; an embeddable relayer lifecycle is not required for the initial use case.
...
import (
	...
	"github.com/cosmos/ibc/cli/lightclient"
...
- Rename `cli/lightclient` to `cli/lightclient`.
- Keep `ProofGenerator`, `Factory`, `Deps`, `ClientEnd`, `RawParams`, and `Registry` public under `cli/lightclient`.
...
- Move root command construction and execution from `cli/cmd/ibc` into an importable `cli/cli` package.
...
- Make `cli/cmd/ibc/main.go` a thin standard entrypoint that calls `cli.Run(cli.Options{})`.
...
- Delete `cli/app/app.go` and its public lifecycle types.
- Move any useful lifecycle tests to the CLI or bootstrap layer.
- Update package documentation so it directs custom-client authors to `cli/lightclient` and `cli/cli`.
- Confirm no production or test package depends on `cli/app` before removal.

### 5. Adapt the end-to-end fixture

- Replace the current in-process `cli/app` test host with a test custom binary built through the public CLI API.
...
Treat `cli/lightclient` and the minimal constructor surface in `cli/cli` as public APIs. Everything below them remains internal and may evolve without supporting downstream imports. Because custom clients are compiled into the binary, downstream implementations must rebuild when upgrading to an incompatible IBC CLI release.
