...
func TestGeneratorPacketProofs(t *testing.T) {
	...
	t.Run("returnsOneProofPerPacket", func(t *testing.T) {
		...
		gen := New("client-chain", "client-0", attestors, 2, nil, slog.Default())
	...
	t.Run("unsupportedKindErrors", func(t *testing.T) {
		...
		gen := New("client-chain", "client-0", nil, 2, nil, slog.Default())
...
func TestGeneratorLatestProvableHeight(t *testing.T) {
	...
	gen := New("client-chain", "client-0", attestors, 2, counterpartyChain, slog.Default())
	...
	for _, kind := range []v2.ProofKind{v2.ProofKindPacketCommitment, v2.ProofKindAcknowledgement, v2.ProofKindReceiptAbsence} {
		for _, tt := range tests {
			t.Run(fmt.Sprintf("%v/%s", kind, tt.name), func(t *testing.T) {
				...
				gen := New("client-chain", "client-0", []attestor.Attestor{
...
func TestGeneratorStateTimestampMismatch(t *testing.T) {
	...
	gen := New(
		"client-chain",
		"client-0",
		[]attestor.Attestor{signedStateAttestor(t, "a1", 10)},
		1,
		chain,
		slog.Default(),
	)
...
func TestGeneratorAcknowledgementProofs(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			gen := New("client-chain", "client-0", attestors, 1, nil, slog.Default())
