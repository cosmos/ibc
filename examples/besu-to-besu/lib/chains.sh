...
_PRINTED_ALLOC=""      # alloc set already spelled out; suppresses a repeat listing
...
_init_chain() {
  ...
  local i funded role privkey alloc_key
  # Both chains default to one mnemonic and one validator index, so the second
  # listing would repeat the first line for line. Print it once and say so.
  alloc_key="${CHAIN_ALLOC_ADDRS[*]}|$addr"
  if [[ "$alloc_key" == "$_PRINTED_ALLOC" ]]; then
    log "[$name] genesis funds the same ${#CHAIN_ALLOC_ADDRS[@]} accounts listed above"
  else
...
_start_chains_env() {
  ...
  {
    echo "# Generated by ./setup.sh — do not edit, do not commit."
    echo "#"
...
start_chains() {
  ...
  for name in "${CHAINS[@]}"; do
    [[ -f "$LOCAL_DIR/$name/key" ]] \
      || die "$LOCAL_DIR/$name/key is missing — the init phase did not complete"
  done
  log "Bringing up besu-a and besu-b..."
  docker compose --progress quiet up -d besu-a besu-b \
    || die "besu-a/besu-b did not start — check 'docker compose logs besu-a besu-b'"
}
