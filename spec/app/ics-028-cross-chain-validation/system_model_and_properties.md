...
- **Staking Safety** Let `UO` be any unbonding operation that starts with an unboding transaction being executed and completes with the event that returns the corresponding stake. Let `vsc(UO)` be the VSC that contains the validator update corresponding to `UO`. Then, `UO` MUST NOT complete on the provider chain before the provider chain registers notifications of `vsc(UO)`'s maturity from all consumer chains. 
