...
- `slashGasMeter: int64` is a meter which stores "slash gas", ie. an amount of voting power % that corresponds to an allowance of validators that can be jailed at any given time. This meter is replenished every hour via an on-chain parameter. A negative value for this meter indicates that the most recently jailed validator had a voting power % that exceeded remaining gas for that hour. Negative values are allowed to enable validators with significant voting power % to be jailed atomically. Once the meter reaches 0 or negative, no more slash packets will be handled until the meter is replenished.
