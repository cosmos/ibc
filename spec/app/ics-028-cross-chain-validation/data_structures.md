...
- `pendingSlashPackets: [SlashPacket]` is a queue of slash packets that have not yet been processed by the staking module on the provider chain. Upon the provider ccv module receiving a slash packet from the core IBC module, that packet will be stored in this queue. The provider's EndBlock routine will dequeue packets from the queue (with appropriate limiting mechanisms) and pass them to the staking module as specified by `handleSlashPacket`.
- `JailMeter` is a meter which stores an amount of voting power (int) that corresponds to an allowance of validators that can be jailed/tombstoned over time. This meter is replenished every `JailMeterReplenishPeriod` via an on-chain parameter. A negative value for this meter indicates that the most recently jailed/tombstoned validator had a voting power that exceeded the remaining voting power stored in the meter. Negative values are allowed, to enable validators with significant voting power to be jailed/tombstoned atomically. Once the meter reaches 0 or negative, no more slash packets that affect voting power will be handled until the meter is replenished to a positive value.
- `JailMeterReplenishPeriod` is an on-chain time period parameter used to determine how often the `JailMeter` will be replenished with voting power allowance in `EndBlockCIS`.
- `JailAllowanceFraction` is an on-chain fraction parameter used to determine the fraction of total voting power that will be replenished every `JailMeterReplenishPeriod`.

TODO: total voting power would change as validators are jailed, is there a timing guarantee we can make on maximum slash packet delay? Perhaps we use a static total voting power for determining the allowance of voting power to be jailed over time (ex: the max total voting power set from tendermint).
