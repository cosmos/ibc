...
- `vscTimeoutTimestamps: Map<string, [uint64]>` is a mapping from consumer chain IDs to order lists of timeout timestamps (see `vscTimeout`), one for every `VSCPacket` sent for which the corresponding `VSCMaturedPacket` was not received.
