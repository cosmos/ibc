...
```typescript
function RegisterInterchainAccount(counterpartyPortId: string) returns (nil) {
...
ICS-27 channels can only be closed in the event of a timeout (if the implementation uses ordered channels) or in the unlikely event of a light client attack. Controller chains must retain the ability to open new ICS-27 channels and reset the active channel for a particular portID (containing `{owner-account-address}`) and associated interchain account.

The new channel must be opened with the same underlying connections as specified in the stored metadata of the previous active channel.
...
function onChanOpenTry(
  ...
  account = RegisterInterchainAccount(counterpartyPortIdentifier)
