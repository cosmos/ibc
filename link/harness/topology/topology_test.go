...
func TestWithManualRelay(t *testing.T) {
	...
	// A caller may bind once and derive more than once; the transform must copy, never mutate its input.
...
// TestValidate drives every check through ad-hoc composed topologies — the path Validate exists for.
