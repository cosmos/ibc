...
func TestAwaitDeadlineCarriesDescAndLastProbeError(t *testing.T) {
	...
	_, err := Await(t.Context(), 40*time.Millisecond, testPoll, "the mint on chain-b", func(context.Context) (int, bool, error) {
...
func TestAwaitDeadlineWithoutProbeError(t *testing.T) {
	_, err := Await(t.Context(), 40*time.Millisecond, testPoll, "the mint on chain-b", func(context.Context) (int, bool, error) {
		return 0, false, nil // nothing observed yet, no error to retain
	})
	require.Error(t, err)
	require.ErrorIs(t, err, context.DeadlineExceeded)
