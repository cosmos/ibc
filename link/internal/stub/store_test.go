...
const (
	testSourceTxHash = "0xsrc"
	testReceiptHash  = "0xreceipt"
)

func packetByID(ctx context.Context, s *stubStore, packetID string) (storedPacket, bool, error) {
	rows, err := s.Packets(ctx, packetID)
	if err != nil || len(rows) == 0 {
		return storedPacket{}, false, err
	}
	return rows[0], true, nil
}

func open(t *testing.T) *stubStore {
	t.Helper()
...
func TestOpenRejectsInMemory(t *testing.T) {
	_, err := openStore(":memory:")
	if err == nil {
		t.Fatal("openStore(:memory:) succeeded; want error")
	}
}
...
func TestTestAppDeploymentRoundTrip(t *testing.T) {
	...
	want := testappcmd.Deployment{
		Chains: map[string]testappcmd.ChainDeployment{
			"31337": {
				MockIFT: "0xaaa", MockGMP: "0xbbb", Counter: "0xccc", TxHash: testReceiptHash,
			},
		},
	}
	...
	if got.Chains["31337"].MockIFT != "0xaaa" || got.Chains["31337"].TxHash != testReceiptHash {
		t.Fatalf("round-trip mismatch: %+v", got)
	}

	want.Chains["31337"] = testappcmd.ChainDeployment{MockIFT: "0xnew"}
	if saveErr := st.SaveTestApps(ctx, want); saveErr != nil {
...
func TestPacketLifecycle(t *testing.T) {
	...
	if got.State != relayercmd.PacketPending || got.SourceTxHash != testSourceTxHash ||
		got.AppType != relayercmd.AppTypeIFT ||
		got.Amount != "42" {
	...
	if got.State != relayercmd.PacketComplete || got.EffectTxHash != "0xrecv" {
...
func TestRelayRequestsAreIdempotent(t *testing.T) {
	...
	if err := st.RequestRelay(ctx, testChainA, testSourceTxHash); err != nil {
	...
	if !got[(relayRequestKey{SourceChainID: testChainA, SourceTxHash: testSourceTxHash})] || len(got) != 1 {
...
func TestPacketsBySourceTxIsCaseInsensitive(t *testing.T) {
	st := open(t)
	ctx := context.Background()
	for _, p := range []storedPacket{
		{PacketID: "p1", RouteID: "r1", AppType: relayercmd.AppTypeIFT, Sequence: 1, SourceTxHash: "ABCDEF"},
		{PacketID: "p2", RouteID: "r1", AppType: relayercmd.AppTypeGMP, Sequence: 2, SourceTxHash: "0xother"},
	} {
		if err := st.InsertPending(ctx, p); err != nil {
			t.Fatal(err)
		}
...
func TestTerminalStatesAreImmutable(t *testing.T) {
	...
	seed := func(id string, mark func() error) {
		...
		if err := st.InsertPending(
			ctx,
			storedPacket{PacketID: id, RouteID: "r1", AppType: relayercmd.AppTypeIFT, Sequence: 1},
		); err != nil {
	...
	stateOf := func(id string) relayercmd.PacketState {
		t.Helper()
	...
	timedOut := relayercmd.PacketID("r1", relayercmd.AppTypeIFT, 1)
	...
	if got := stateOf(timedOut); got != relayercmd.PacketTimedOut {
		t.Fatalf("timed_out packet regressed to %q", got)
	}

	errAck := relayercmd.PacketID("r1", relayercmd.AppTypeGMP, 1)
	...
	if got := stateOf(errAck); got != relayercmd.PacketErrorAck {
		t.Fatalf("error_ack packet regressed to %q", got)
	}

	done := relayercmd.PacketID("r2", relayercmd.AppTypeIFT, 1)
	...
	if got := stateOf(done); got != relayercmd.PacketComplete {
