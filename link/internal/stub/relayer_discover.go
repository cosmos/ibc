...
func (r *relayer) discoverEVMSourceTx(
	...
	chainID string,
	txHash string,
	chainApps testappcmd.ChainDeployment,
) error {
	...
	ift := newTestAppIFT(common.HexToAddress(chainApps.MockIFT), conn.client)
	...
	gmp := newTestAppGMP(common.HexToAddress(chainApps.MockGMP), conn.client)
...
func (r *relayer) insertEVMIFT(ctx context.Context, chainID string, event iftSent) error {
	...
	return r.store.InsertPending(ctx, storedPacket{
		PacketID:         relayercmd.PacketID(route.ID, relayercmd.AppTypeIFT, event.Seq.Uint64()),
		RouteID:          route.ID,
...
func (r *relayer) insertEVMGMP(ctx context.Context, chainID string, event gmpSent) error {
	...
	return r.store.InsertPending(ctx, storedPacket{
		PacketID:     relayercmd.PacketID(route.ID, relayercmd.AppTypeGMP, event.Seq.Uint64()),
		RouteID:      route.ID,
