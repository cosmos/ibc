...
// RelayerRun returns the temporary synthetic Relayer handler.
func RelayerRun(flags *internalconfig.FlagSet) relayercmd.Handler {
	return func(cmd *cobra.Command, _ []string, _ relayercmd.RunOptions) error {
...
func runRelayer(cmd *cobra.Command, flags *internalconfig.FlagSet) error {
	...
	if storeErr := requireStore(c); storeErr != nil {
		return storeErr
	}
	...
	if err != nil {
		if errors.Is(err, errNoTestApps) {
			return fmt.Errorf("%w (db %q)", err, c.DB.URL)
		}
		return err
	}
