...
binary (`../cmd/ibc`) does not yet satisfy.
...
- **The stub is a swap ledger entry, not a product.** Each `Driver` method in
  `../harness/ibclink/runner.go` selects the real or stub binary; long-lived command selection lives
  in `daemon.go`. Once the real implementation satisfies the e2e wire contract, change that
  method's binary selection, delete the stub piece it replaces, and keep every lane green. The stub
  SHOULD import real link packages
