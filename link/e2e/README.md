...
routes each wire command per the routing table in `internal/harness/ibclink/runner.go` (`IBC_BIN` /
...
- Assert packet outcomes through their harness methods (`VerifyComplete`, `VerifyPendingStable`,
  `VerifyTimedOut`, `VerifyErrorAck`), never via `internal/harness/ibclink/wire` or the daemon directly.
...
- Ad-hoc composition (`./external`, the cross-route test) — build `ChainSpec`s directly for an
  ...
  the reserved-ranges note in `internal/harness/topology/lane.go`. A shape constructor exists only once more
