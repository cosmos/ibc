...
These tests drive IBC Link as a black box through its public CLI, config, readiness, relay, and status contracts. The accepted harness design is documented in [IBC Environment Architecture](../HARNESS-ARCHITECTURE-DESIGN.md); the synthetic traffic in this suite is transitional and does not shape that architecture.

`harness/environment` realizes Chains and protocol resources. Tests then deploy the temporary MockIFT, MockGMP, and Counter applications and start the synthetic relayer explicitly, so process restarts, manual relay, fault injection, and teardown remain visible in the behavior under test.
...
`make build` produces `bin/ibc`; `make build-stub` produces the temporary synthetic implementation at `bin/ibc-stub`. `IBC_BIN` and `IBC_STUB_BIN` override those paths.
