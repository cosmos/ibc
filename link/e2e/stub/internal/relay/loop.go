...
// relayer holds the loop's state: config, deployment, dialed chains, store, and
...
// FindReceived as the idempotency guard. iftDelivery adds the source-side refund for the timeout leg.
...
func (r *relayer) iftDelivery(
	...
	// Add the source-side refund for the timeout leg: the source is an EVM MockIFT the relayer
