...
- Replaces the prover service's `StateProof` and `PacketProofs` RPCs with `Prepare`, which returns either a client-only checkpoint to confirm first or the update and packet proofs for a batch. Remote prover services must implement `Prepare`.
