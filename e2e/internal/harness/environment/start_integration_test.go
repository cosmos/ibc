...
func TestStartRealizesSolidityIBCAppStackConnectionAndAttestors(t *testing.T) {
	...
	resolvedAttestorA, err := env.Attestor(attestorA)
	require.NoError(t, err)
	resolvedAttestorB, err := env.Attestor(attestorB)
	require.NoError(t, err)
	require.Same(t, connection.A(), resolvedAttestorA.IBCClient())
	require.Same(t, resolvedInstanceB, resolvedAttestorA.ObservedIBCInstance())
	require.Same(t, connection.B(), resolvedAttestorB.IBCClient())
	require.Same(t, resolvedInstanceA, resolvedAttestorB.ObservedIBCInstance())
	require.NotEqual(t, resolvedAttestorA.SignerAddress(), resolvedAttestorB.SignerAddress())
	require.Equal(t, []environment.EVMAddress{resolvedAttestorA.SignerAddress()}, connection.A().AttestorAddresses())
	require.Equal(t, []environment.EVMAddress{resolvedAttestorB.SignerAddress()}, connection.B().AttestorAddresses())
	require.EqualValues(t, 1, connection.A().MinRequiredSignatures())
	require.EqualValues(t, 1, connection.B().MinRequiredSignatures())

	endpoint := resolvedAttestorA.Endpoint()
	require.NotEmpty(t, endpoint)
	require.NotContains(t, endpoint, "://")
	height, err := resolvedAttestorA.LatestHeight(t.Context())
	require.NoError(t, err)
	require.NotZero(t, height)

	// Restart itself fails unless the child announces the original endpoint.
	require.NoError(t, resolvedAttestorA.Restart(t.Context()))
	restartedHeight, err := resolvedAttestorA.LatestHeight(t.Context())
	require.NoError(t, err)
	require.GreaterOrEqual(t, restartedHeight, height)

	require.NoError(t, resolvedAttestorA.Stop(t.Context()))
	_, err = resolvedAttestorA.LatestHeight(t.Context())
	require.Error(t, err)
...
func requireIBCLinkBinary(t *testing.T) {
	...
	if err != nil {
		...
		t.Skipf("IBC Link binary is unavailable at %s: %v; run `just link::build`", path, err)
	}
	if info.Mode()&0o111 == 0 {
		if os.Getenv("IBC_BIN") != "" {
			require.FailNow(t, "explicit IBC_BIN is not executable: "+path)
		}
		t.Skipf("IBC Link binary is not executable: %s; run `just link::build`", path)
	}
}
