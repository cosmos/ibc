...
// Package ibccli drives the IBC CLI SUT as a black box over its public CLI, config YAML, and JSON output.
package ibccli
...
func (r *Driver) ConfigureRelayer(opts RelayerOptions) error {
	for route, policy := range opts.WaitPolicies {
		if policy.CompletionBudget <= 0 || policy.StatusPoll <= 0 || policy.StabilityWindow <= 0 {
			return fmt.Errorf("ibccli: route %q has invalid wait policy: %+v", route, policy)
		}
	}
	r.relayerOpts = opts
	return nil
...
func NewDriver(configPath string) (*Driver, error) {
	if configPath == "" {
		return nil, errors.New("ibccli: NewDriver requires a config path")
	}
	return &Driver{
		bin:        ResolvedBin(),
