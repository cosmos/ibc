...
const (
	chainRPCEnvPrefix = "IBC_CLI_CHAIN_RPC_"
	chainWSEnvPrefix  = "IBC_CLI_CHAIN_WS_"
)
...
func (r *Driver) BindChainEndpoints(
	...
	if acquire == nil {
		return errors.New("ibccli: process binding lease is required")
	}
	chains := make(map[string]chainEndpointBinding, len(resolvers))
	for chainID, resolve := range resolvers {
		if chainID == "" {
			return errors.New("ibccli: cannot bind an empty Chain id")
		}
		if resolve == nil {
			return fmt.Errorf("ibccli: Chain %q endpoint resolver is required", chainID)
		}
	...
	if r.bindings != nil {
		return errors.New("ibccli: Chain RPC bindings are already installed")
	}
...
func (r *Driver) chainBinding(chainID string) (chainEndpointBinding, error) {
	if r.bindings == nil {
		return chainEndpointBinding{}, fmt.Errorf("ibccli: no RPC binding for Chain %q", chainID)
	}
...
func (r *Driver) acquireProcessEnv() ([]string, func(), error) {
	...
	if release == nil {
		return nil, nil, errors.New("ibccli: process binding lease returned no release function")
	}
...
func resolveChainEndpoints(chainID string, binding chainEndpointBinding) (ChainEndpoints, error) {
	...
	if err != nil {
		return ChainEndpoints{}, fmt.Errorf("ibccli: resolve RPC binding for Chain %q: %w", chainID, err)
	}
	if endpoints.RPC == "" {
		return ChainEndpoints{}, fmt.Errorf("ibccli: RPC binding for Chain %q is empty", chainID)
	}
