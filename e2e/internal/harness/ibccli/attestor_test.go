...
func TestCLIAttestorHelperProcess(_ *testing.T) {
	if os.Getenv("IBC_CLI_ATTESTOR_HELPER") != "1" {
		return
	}
...
func helperBinary(t *testing.T) string {
	...
	require.NoError(t, err)
	t.Setenv("IBC_CLI_ATTESTOR_TEST_BINARY", testBinary)

	path := filepath.Join(t.TempDir(), "ibc-helper")
	script := `#!/bin/sh
IBC_CLI_ATTESTOR_HELPER=1 exec "$IBC_CLI_ATTESTOR_TEST_BINARY" -test.run '^TestCLIAttestorHelperProcess$' -- "$@"
`
